<?php 
class us_serah_stnk extends user_controller{
	var $controller;
    
    
	function us_serah_stnk(){
		parent::__construct();

		$this->controller = get_class($this);
		$this->load->model('us_serah_stnk_model','dm');
...
function index(){
		...
		$this->set_subtitle("Penyerahan STNK Ke-Samsat");
		$this->set_title("STNK");
 ...
 function cekbiaya(){
    ...
    foreach ($post['data'] as $key => $value) {
        ...
        $biaya = $biaya + $check['rp_daftar_stnk'];
...
if(!empty($chb)) { 
    ...
    $cek_no = $this->db->get('no_ref_stnk');
    if ($cek_no->num_rows()<1) {
        $ref = array('no_terakhir' => 1,
                    'id_birojasa' => $userdata['birojasa_id'], );
        $tambah_ref = $this->db->insert('no_ref_stnk', $ref);
    }else{
        $no_terakhir = $cek_no->row_array();
        $no_baru = $no_terakhir['no_terakhir']+1;
        $ref = array('no_terakhir' => $no_baru );
        $this->db->where('id_birojasa', $userdata['birojasa_id']);
        $update_ref = $this->db->update('no_ref_stnk', $ref);
    }


    $no_ref_stnk = 'REF.'.sprintf("%06d", $ref['no_terakhir']).'/STNK'; 
    $tanggal = date("Y-m-d");

    $data_ref_stnk = array('no_ref' => $no_ref_stnk,
                            'jumlah_berkas' => count($chb),
                            'tanggal' => $tanggal,
                            'id_user' => $userdata['id'],
                            'id_birojasa' => $userdata['birojasa_id'],
                            'id_samsat' => $id_samsat );
        $res = $this->db->insert('ref_stnk', $data_ref_stnk);
