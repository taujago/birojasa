<?php 
class bj_invoice_bpkb extends biro_jasa_controller{
	var $controller;
	function bj_invoice_bpkb(){
		parent::__construct();

		$this->controller = get_class($this);
		$this->load->model('bj_invoice_bpkb_model','dm');
	...
	function index(){
		...
		$this->set_subtitle("Invoice BPKB");
...
function cekbiaya(){
    ...
    foreach ($post['data'] as $key => $value) {
        ...
        $biaya = $biaya + $check['bayar_jumlah_bpkb'];
 ...
 function simpan(){
    ...
    $this->form_validation->set_rules('dealer','Pilih Dealer','required');
    $this->form_validation->set_rules('diterima','Penerima','required');
    $this->form_validation->set_rules('dibuat','Pembuat','required');
...
if(!empty($chb)) { 
    ...
    $cek_no = $this->db->get('no_invoice_bpkb');
    if ($cek_no->num_rows()<1) {
        $ref = array('no_terakhir' => 1,
                    'id_birojasa' => $userdata['birojasa_id'], );
        $tambah_ref = $this->db->insert('no_invoice_bpkb', $ref);
    }else{
        $no_terakhir = $cek_no->row_array();
        $no_baru = $no_terakhir['no_terakhir']+1;
        $ref = array('no_terakhir' => $no_baru );
        $this->db->where('id_birojasa', $userdata['birojasa_id']);
        $update_ref = $this->db->update('no_invoice_bpkb', $ref);
    }


    $no_invoice_bpkb = 'INV/'.sprintf("%06d", $ref['no_terakhir']).'/BPKB';
    $no_tt_bpkb = 'TT/'.sprintf("%06d", $ref['no_terakhir']).'/BPKB'; 
    $tanggal = date("Y-m-d");

    $data_inv_bpkb = array('no_invoice' => $no_invoice_bpkb,
                            'jumlah_berkas' => count($chb),
                            'tanggal' => $tanggal,
                            'id_birojasa' => $userdata['birojasa_id'],
                            'id_dealer' => $kode_dealer, 
                            'no_tt_bpkb' => $no_tt_bpkb,
    ...
    // exit();
        ...
        $res = $this->db->insert('invoice_bpkb', $data_inv_bpkb);
         
        if($res){

            $data_update = array('no_invoice_bpkb' => $no_invoice_bpkb, 
                                'bpkb_serah_dealer' => 1);
