<?php 
class us_serah_stck_detail extends user_controller{
	var $controller;
    
    
	function us_serah_stck_detail(){
		parent::__construct();

		$this->controller = get_class($this);
		$this->load->model('us_serah_stck_detail_model','dm');
	...
	function index(){
		...
		$this->set_subtitle("Detail Penyerahan STCK Ke-Samsat");
		$this->set_title("STCK");
...
function hapusdata(){
        ...
        $data = array('id' => $id, );
        $this->db->select('no_ref_stck');
        $this->db->where('id', $id);
        $data_bbn = $this->db->get('bj_bbn_satu')->row_array();
        $no_ref = $data_bbn['no_ref_stck'];
        // echo $no_ref;
        ...
        $update_data = array('status_stck' => 0,
                            'status_terima_stck' => 0,
                            'status_terima_stck' => 0,
                            'jumlah_bayar_stck' => 0, 
                            'no_ref_stck' => ''
                             );
        ...
        if($res){
            ...
            $this->db->update('ref_stck', $update_ref);
	...
	function get_data(){
      ...
      //  order[0][column]
        ...
        $no_ref = $row['no_ref'];
         ...
         $nama ="<b><a href='us_serah_stck_detail/lihatdata?id=$id'>$no_ref</a></b>";
    ...
    function lihatdata(){
    	...
    	$ref_stck = $this->db->get('ref_stck')->row_array();
    ...
    function pdf(){
    	...
    	$this->db->select('s.*, sa.nama as nm_samsat')->from("ref_stck s");
        $this->db->join('samsat sa','s.id_samsat=sa.id');
    	$this->db->where('s.id', $get['id']);
    	$ref_stck = $this->db->get('')->row_array();
    ...
    $data['controller'] = get_class($this);
    $data['header'] = "Detail Penyerahan Berkas STCK ";
